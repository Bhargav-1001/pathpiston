...
export default async function RideDetailsPage({ params }: { params: { id: number } }) {
    const { id } = await params;
    const prisma = new PrismaClient();

    const ridedetailsbyId = await prisma.rideEvent.findUnique({ include: { participants: true, category: true }, where: { id: Number(id) } })
    const participantdetailsbyId = await prisma.rideParticipant.findUnique({
        include: { user: true },
        where: { id: Number(ridedetailsbyId?.id) }
    })
    return (
        <div>
            <div className="row">
                <div className="col-lg-8 mb-4">
                    <div className="card card-dark">
                        <img
                            src={ridedetailsbyId?.photo || "/placeholder.svg?height=400&width=800"}
                            className="card-img-top"
                            alt="image not found"
                            style={{ height: "400px", objectFit: "cover" }}
                        />
                        <div className="card-body">
                            <div className="d-flex justify-content-between align-items-start mb-3">
                                <div>
                                    <div className="mb-2">
                                        <span className="badge bg-secondary me-2">{ridedetailsbyId?.category.name}</span>
                                        <span className="badge bg-danger">{ridedetailsbyId?.difficultyLevel}</span>
                                    </div>
                                    <h2 className="mb-2">{ridedetailsbyId?.title}</h2>
                                    {/* <div className="text-muted">
                                        Organized by <strong>Mountain Riders Club</strong>
                                    </div> */}
                                </div>
                                <div className="text-end">
                                    <div className="h3 text-accent mb-0">{ridedetailsbyId?.registrationFees}</div>
                                    <div className="small text-muted">per rider</div>
                                </div>
                            </div>

                            <p className="mb-4">
                                {ridedetailsbyId?.description}
                            </p>
                            ...
                            <div className="row mb-4">
                                <div className="col-md-6">
                                    <div className="mb-3">
                                        ...
                                        <div className="ms-4 text-muted">
                                            {ridedetailsbyId?.date
                                                ? `${new Date(ridedetailsbyId.date).toLocaleDateString()} at ${ridedetailsbyId?.time || ""}`
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <div className="ms-4 text-muted">{ridedetailsbyId?.meetingPoint}</div>
                                    </div>
                                </div>
                                ...
                                <div className="col-md-6">
                                    <div className="mb-3">
                                        ...
                                        <div className="ms-4 text-muted">{ridedetailsbyId?.destination}</div>
                                    </div>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <div className="ms-4 text-muted">{ridedetailsbyId?.estimatedKm} Km</div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="col-lg-4">
                    <div className="card card-dark mb-4">
                        ...
                        <div className="card card-dark mb-4">
                            <div className="card-header">
                                <h6 className="mb-0">
                                    {ridedetailsbyId?.participants.length}
                                </h6>
                            </div>
                            <div className="card-body">

                                <div className="d-flex align-items-center mb-3">
                                    <img src={participantdetailsbyId?.user.profilePhoto || "/placeholder.svg?height=40&width=40"} alt="Rajesh Kumar" className="avatar me-3" />
                                    <div className="flex-grow-1">
                                        <div className="fw-semibold">{participantdetailsbyId?.user.name}</div>
                                        <div className="small text-muted">
                                            <span className="badge badge-difficulty bg-danger me-2">
                                                {ridedetailsbyId?.difficultyLevel}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="card card-dark">
                            <div className="card-body text-center">
                                <button className="btn btn-accent btn-lg w-100 mb-3">
                                    Join This Ride
                                </button>
